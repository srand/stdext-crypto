...
public:
  detail() { SHA1_Init(&_ctx); }

  void update(const void *data, size_t len) { SHA1_Update(&_ctx, data, len); }
...
public:
  detail() { SHA256_Init(&_ctx); }

  void update(const void *data, size_t len) { SHA256_Update(&_ctx, data, len); }
...
public:
  hashsum(DWORD provider, DWORD algo) : _provider(0), _hash(0) {
    if (!CryptAcquireContext(&_provider, NULL, NULL, provider, 0)) {
...
sha1sum::sha1sum() : _impl(new sha1sum::detail()) {}
...
sha256sum::sha256sum() : _impl(new sha256sum::detail()) {}
